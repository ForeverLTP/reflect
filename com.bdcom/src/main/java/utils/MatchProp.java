...
public class MatchProp {
	...
	public static List<Object> matchProp(ResultSet rs, Class<?> className)
			throws SQLException, ParseException, InstantiationException, IllegalAccessException {
	...
	public static List<Object> matchPropForMutil(ResultSet rs, Class<?> className)
			throws SQLException, ParseException, InstantiationException, IllegalAccessException {
		ResultSetMetaData rsm = rs.getMetaData();// 获得列集
		int count = rsm.getColumnCount();
		map = new HashMap<String, Object>(count);
		...
		while (rs.next()) {
			...
			Field[] fields = className.getDeclaredFields();// 获取该class的所有属性
			proMap = new HashMap<String,Field>(fields.length);
			for(Field field:fields){
				String fName = field.getName();
				proMap.put(fName.toLowerCase(), field);
			}
			//更改选择模式(利用查询出来的属性进行匹配数据)
			for(String key:map.keySet()){
				Field field = proMap.get(key);
				Class<?> type = field.getType();// 获取实例对象中的属性的类型
				field.setAccessible(true);// 将属性设置为可以编辑的状态
